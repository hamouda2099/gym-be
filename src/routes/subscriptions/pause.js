...
router.put("/subscriptions/pause/:id", async (req, res) => {
  try {
    ...
    if (tokenData.statusCode === 200) {
      var resBody = await pauseSubscription(req.params.id,req.body);
...
async function pauseSubscription(id,body) {
  var resBody = {};
  ...
  await db
    .execute(`${q};`)
    .then((result) => {
      resBody = { statusCode: 200, message: "paused successfully" };
    })
    .catch((err) => {
      console.log(err);
