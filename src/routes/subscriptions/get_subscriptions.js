...
router.get("/subscriptions", async (req, res) => {
  try {
    ...
    if (tokenData.statusCode === 200) {
      var resBody = await getSubscriptions(
        req.query.page,
        req.query.limit,
        req.query.query
      );
...
async function getSubscriptions(page, limit, query) {
  var resBody = {};
  var sqlQuery = queries.craeteQuery(
    ...
    [
      ...
      { column: "phone", value: query, key: "LIKE" },
    ],
    "deleted_at"
  );
  ...
  await db
    .execute(
      `SELECT * FROM subscriptions ${sqlQuery.query} ORDER BY created_at DESC LIMIT ${sqlQuery.from},${sqlQuery.limit};`
    )
    .then((result) => {
      resBody = { statusCode: 200, message: "subscribtions found", subscribtions: result[0] };
    })
    .catch((error) => {
      console.log(error);
