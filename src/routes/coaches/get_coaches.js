...
async function getCoaches(page, limit, query) {
  var resBody = {};
  ...
  await db
    .execute(
      `SELECT * FROM coaches ${sqlQuery.query} ORDER BY created_at DESC LIMIT ${sqlQuery.from},${sqlQuery.limit};`
    )
    .then((result) => {
      resBody = { statusCode: 200, message: "coaches found", coaches: result[0] };
    })
    .catch((error) => {
      console.log(error);
